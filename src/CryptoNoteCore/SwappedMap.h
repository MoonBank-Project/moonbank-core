...
private:
  ...
  std::list<Key> m_moonbank;
  std::unordered_map<Key, typename std::list<Key>::iterator> m_moonbankIterators;
  uint64_t m_moonbankHits;
  uint64_t m_moonbankMisses;
...
template<class Key, class T> void SwappedMap<Key, T>::close() {
  //std::cout << "SwappedMap moonbank hits: " << m_moonbankHits << ", misses: " << m_moonbankMisses << " (" << std::fixed << std::setprecision(2) << static_cast<double>(m_moonbankMisses) / (m_moonbankHits + m_moonbankMisses) * 100 << "%)" << std::endl;
}
...
template<class Key, class T> void SwappedMap<Key, T>::erase(const_iterator iterator) {
  ...
  auto moonbankIteratorsIterator = m_moonbankIterators.find(descriptorsIterator->first);
  if (moonbankIteratorsIterator != m_moonbankIterators.end()) {
    m_items.erase(descriptorsIterator->first);
    m_moonbank.erase(moonbankIteratorsIterator->second);
    m_moonbankIterators.erase(moonbankIteratorsIterator);
  }
}
...
template<class Key, class T> std::pair<const Key, T>* SwappedMap<Key, T>::prepare(const Key& key) {
  if (m_items.size() == m_poolSize) {
    typename std::list<Key>::iterator moonbankIter = m_moonbank.begin();
    m_items.erase(*moonbankIter);
    m_moonbankIterators.erase(*moonbankIter);
    m_moonbank.erase(moonbankIter);
  }

  std::pair<typename std::unordered_map<Key, T>::iterator, bool> itemInsert = m_items.insert(std::make_pair(key, T()));
  typename std::list<Key>::iterator moonbankIter = m_moonbank.insert(m_moonbank.end(), key);
  m_moonbankIterators.insert(std::make_pair(key, moonbankIter));
...
template<class Key, class T> const std::pair<const Key, T>* SwappedMap<Key, T>::load(const Key& key, uint64_t offset) {
  ...
  if (itemIterator != m_items.end()) {
    auto moonbankIteratorsIterator = m_moonbankIterators.find(key);
    if (moonbankIteratorsIterator->second != --m_moonbank.end()) {
      m_moonbank.splice(m_moonbank.end(), m_moonbank, moonbankIteratorsIterator->second);
    }
