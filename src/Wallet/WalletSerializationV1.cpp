...
void WalletSerializer::save(const std::string& password, Common::IOutputStream& destination, bool saveDetails, bool saveMoonBank) {
  ...
  saveWallets(destination, saveMoonBank, cryptoContext);
  saveFlags(saveDetails, saveMoonBank, destination, cryptoContext);
  ...
  if (saveMoonBank) {
    saveBalances(destination, saveMoonBank, cryptoContext);
...
void WalletSerializer::saveFlags(bool saveDetails, bool saveMoonBank, Common::IOutputStream& destination, CryptoContext& cryptoContext) {
  serializeEncrypted(saveDetails, "details", cryptoContext, destination);
  cryptoContext.incIv();

  serializeEncrypted(saveMoonBank, "moonbank", cryptoContext, destination);
  cryptoContext.incIv();
}

void WalletSerializer::saveWallets(Common::IOutputStream& destination, bool saveMoonBank, CryptoContext& cryptoContext) {
  ...
  for (const auto& w: index) {
    ...
    dto.pendingBalance = saveMoonBank ? w.pendingBalance : 0;
    dto.actualBalance = saveMoonBank ? w.actualBalance : 0;
...
void WalletSerializer::saveBalances(Common::IOutputStream& destination, bool saveMoonBank, CryptoContext& cryptoContext) {
  uint64_t actual = saveMoonBank ? m_actualBalance : 0;
  uint64_t pending = saveMoonBank ? m_pendingBalance : 0;
...
void WalletSerializer::loadWallet(Common::IInputStream &source, const Crypto::chacha8_key &key, uint32_t version)
{
  ...
  loadFlags(details, moonbank, source, cryptoContext);
...
void WalletSerializer::loadFlags(bool& details, bool& moonbank, Common::IInputStream& source, CryptoContext& cryptoContext) {
  deserializeEncrypted(details, "details", cryptoContext, source);
  cryptoContext.incIv();

  deserializeEncrypted(moonbank, "moonbank", cryptoContext, source);
  cryptoContext.incIv();
}
...
void WalletSerializer::resetMoonBankdBalance() {
