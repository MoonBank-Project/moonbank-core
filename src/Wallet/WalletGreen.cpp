...
namespace CryptoNote
{
  ...
  void WalletGreen::saveWalletMoonBank(ContainerStorage &storage, const Crypto::chacha8_key &key, WalletSaveLevel saveLevel, const std::string &extra)
  {
    m_logger(INFO) << "Saving moonbank...";
  ...
  void WalletGreen::save(WalletSaveLevel saveLevel, const std::string &extra)
  {
    ...
    try
    {
      saveWalletMoonBank(m_containerStorage, m_key, saveLevel, extra);
    }
    catch (const std::exception &e)
    {
  ...
  void WalletGreen::exportWallet(const std::string &path, bool encrypt, WalletSaveLevel saveLevel, const std::string &extra)
  {
    ...
    try
    {
      ...
      saveWalletMoonBank(newStorage, newStorageKey, saveLevel, extra);
  ...
  void WalletGreen::convertAndLoadWalletFile(const std::string &path, std::ifstream &&walletFileStream)
  {
    ...
    saveWalletMoonBank(m_containerStorage, m_key, WalletSaveLevel::SAVE_ALL, "");
  ...
  void WalletGreen::loadWalletMoonBank(std::unordered_set<Crypto::PublicKey> &addedKeys, std::unordered_set<Crypto::PublicKey> &deletedKeys, std::string &extra)
  {
    ...
    m_logger(INFO) << "Container moonbank loaded";
  }
    ...
    else
    {
      ...
      if (m_containerStorage.suffixSize() > 0)
      {
        try
        {
          ...
          loadWalletMoonBank(addedSpendKeys, deletedSpendKeys, extra);

          if (!addedSpendKeys.empty())
          {
            m_logger(WARNING, BRIGHT_YELLOW) << "Found addresses not saved in container moonbank. Resynchronize container";
            clearMoonBanks(false, true);
            subscribeWallets();
          }

          if (!deletedSpendKeys.empty())
          {
            m_logger(WARNING, BRIGHT_YELLOW) << "Found deleted addresses saved in container moonbank. Remove its transactions";
            deleteOrphanTransactions(deletedSpendKeys);
          }

          if (!addedSpendKeys.empty() || !deletedSpendKeys.empty())
          {
            saveWalletMoonBank(m_containerStorage, m_key, WalletSaveLevel::SAVE_ALL, extra);
          }
        }
        catch (const std::exception &e)
        {
          m_logger(ERROR, BRIGHT_RED) << "Failed to load moonbank: " << e.what() << ", reset wallet data";
          clearMoonBanks(true, true);
          subscribeWallets();
        }
      }
  ...
  void WalletGreen::clearMoonBanks(bool clearTransactions, bool clearMoonBankdData)
  {
      ...
      bool send)
  {
    ...
    m_fusionTxsMoonBank.emplace(transactionId, isFusion);
      ...
      const std::vector<ContainerAmounts> &containerAmountsList)
  {
    ...
    else
    {
      ...
      m_fusionTxsMoonBank.emplace(transactionId, isFusionTransaction(*it));
    }
  ...
  bool WalletGreen::isFusionTransaction(size_t transactionId) const
  {
    ...
    auto isFusionIter = m_fusionTxsMoonBank.find(transactionId);
    if (isFusionIter != m_fusionTxsMoonBank.end())
    {
      return isFusionIter->second;
    }

    bool result = isFusionTransaction(m_transactions.get<RandomAccessIndex>()[transactionId]);
    m_fusionTxsMoonBank.emplace(transactionId, result);
    return result;
  }
  ...
  void WalletGreen::clearMoonBankAndShutdown()
  {
    if (m_walletsContainer.size() != 0)
    {
      m_synchronizer.unsubscribeConsumerNotifications(m_viewPublicKey, this);
  ...
  void WalletGreen::updateInternalMoonBank() {
