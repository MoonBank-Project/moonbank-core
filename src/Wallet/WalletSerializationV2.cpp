...
void WalletSerializerV2::loadKeyListAndBanalces(CryptoNote::ISerializer& serializer, bool saveMoonBank) {
  size_t walletCount;
  ...
  std::unordered_set<Crypto::PublicKey> moonbankdKeySet;
  ...
  for (auto wallet : index) {
    if (moonbankdKeySet.count(wallet.spendPublicKey) == 0) {
      m_addedKeys.insert(wallet.spendPublicKey);
    }
  }
}

void WalletSerializerV2::saveKeyListAndBanalces(CryptoNote::ISerializer& serializer, bool saveMoonBank) {
