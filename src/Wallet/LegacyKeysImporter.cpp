...
void importLegacyKeys(const std::string& legacyKeysFilename, const std::string& password, std::ostream& destination) {
  ...
  CryptoNote::WalletUserTransactionsMoonBank transactionsMoonBank;
  std::string moonbank;
  CryptoNote::WalletLegacySerializer importer(account, transactionsMoonBank);
  importer.serialize(destination, password, false, moonbank);
}
