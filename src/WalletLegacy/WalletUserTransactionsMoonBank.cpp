...
#include "WalletLegacy/WalletUserTransactionsMoonBank.h"
...
WalletUserTransactionsMoonBank::WalletUserTransactionsMoonBank(uint64_t mempoolTxLiveTime) : m_unconfirmedTransactions(mempoolTxLiveTime) {
}

bool WalletUserTransactionsMoonBank::serialize(CryptoNote::ISerializer& s) {
...
void WalletUserTransactionsMoonBank::deserializeLegacyV1(CryptoNote::ISerializer& s) {
  s(m_transactions, "transactions");
  s(m_transfers, "transfers");
...
void WalletUserTransactionsMoonBank::pushToPaymentsIndex(const PaymentId& paymentId, Offset distance) {
  m_paymentsIndex[paymentId].push_back(distance);
}

void WalletUserTransactionsMoonBank::popFromPaymentsIndex(const PaymentId& paymentId, Offset distance) {
...
void WalletUserTransactionsMoonBank::rebuildPaymentsIndex() {
...
uint64_t WalletUserTransactionsMoonBank::unconfirmedTransactionsAmount() const {
  return m_unconfirmedTransactions.countUnconfirmedTransactionsAmount();
}

uint64_t WalletUserTransactionsMoonBank::unconfrimedOutsAmount() const {
  return m_unconfirmedTransactions.countUnconfirmedOutsAmount();
}

uint64_t WalletUserTransactionsMoonBank::countUnconfirmedCreatedDepositsSum() const {
  return m_unconfirmedTransactions.countCreatedDepositsSum();
}

uint64_t WalletUserTransactionsMoonBank::countUnconfirmedSpentDepositsProfit() const {
  return m_unconfirmedTransactions.countSpentDepositsProfit();
}

uint64_t WalletUserTransactionsMoonBank::countUnconfirmedSpentDepositsTotalAmount() const {
  return m_unconfirmedTransactions.countSpentDepositsTotalAmount();
}

size_t WalletUserTransactionsMoonBank::getTransactionCount() const {
  return m_transactions.size();
}

size_t WalletUserTransactionsMoonBank::getTransferCount() const {
  return m_transfers.size();
}

size_t WalletUserTransactionsMoonBank::getDepositCount() const {
  return m_deposits.size();
}

TransactionId WalletUserTransactionsMoonBank::addNewTransaction(uint64_t amount,
                                                             uint64_t fee,
...
void WalletUserTransactionsMoonBank::updateTransaction(
...
void WalletUserTransactionsMoonBank::updateTransactionSendingState(TransactionId transactionId, std::error_code ec) {
  auto& txInfo = m_transactions.at(transactionId);
  if (ec) {
...
std::deque<std::unique_ptr<WalletLegacyEvent>> WalletUserTransactionsMoonBank::onTransactionUpdated(const TransactionInformation& txInfo,
                                                                                                 int64_t txBalance,
...
std::deque<std::unique_ptr<WalletLegacyEvent>> WalletUserTransactionsMoonBank::onTransactionDeleted(const Crypto::Hash& transactionHash) {
  TransactionId id = CryptoNote::WALLET_LEGACY_INVALID_TRANSACTION_ID;
...
std::vector<Payments> WalletUserTransactionsMoonBank::getTransactionsByPaymentIds(const std::vector<PaymentId>& paymentIds) const {
...
std::vector<DepositId> WalletUserTransactionsMoonBank::unlockDeposits(const std::vector<TransactionOutputInformation>& transfers) {
...
std::vector<DepositId> WalletUserTransactionsMoonBank::lockDeposits(const std::vector<TransactionOutputInformation>& transfers) {
...
TransactionId WalletUserTransactionsMoonBank::findTransactionByTransferId(TransferId transferId) const
{
  TransactionId id;
...
bool WalletUserTransactionsMoonBank::getTransaction(TransactionId transactionId, WalletLegacyTransaction& transaction) const
{
...
bool WalletUserTransactionsMoonBank::getTransfer(TransferId transferId, WalletLegacyTransfer& transfer) const
{
...
bool WalletUserTransactionsMoonBank::getDeposit(DepositId depositId, Deposit& deposit) const {
...
Deposit& WalletUserTransactionsMoonBank::getDeposit(DepositId depositId) {
  assert(depositId < m_deposits.size());

  return m_deposits[depositId].deposit;
}

TransactionId WalletUserTransactionsMoonBank::insertTransaction(WalletLegacyTransaction&& Transaction) {
  m_transactions.emplace_back(std::move(Transaction));
  return m_transactions.size() - 1;
}

TransactionId WalletUserTransactionsMoonBank::findTransactionByHash(const Hash& hash) {
...
bool WalletUserTransactionsMoonBank::isUsed(const TransactionOutputInformation& out) const {
  return m_unconfirmedTransactions.isUsed(out);
}

WalletLegacyTransaction& WalletUserTransactionsMoonBank::getTransaction(TransactionId transactionId) {
  return m_transactions.at(transactionId);
}

TransferId WalletUserTransactionsMoonBank::insertTransfers(const std::vector<WalletLegacyTransfer>& transfers) {
  std::copy(transfers.begin(), transfers.end(), std::back_inserter(m_transfers));
  return m_transfers.size() - transfers.size();
}

void WalletUserTransactionsMoonBank::updateUnconfirmedTransactions() {
...
WalletLegacyTransfer& WalletUserTransactionsMoonBank::getTransfer(TransferId transferId) {
  return m_transfers.at(transferId);
}

bool WalletUserTransactionsMoonBank::getTransactionByHash(const Crypto::Hash& hash, WalletLegacyTransaction& transaction) const
{
  TransactionId id = CryptoNote::WALLET_LEGACY_INVALID_TRANSACTION_ID;
  auto it = std::find_if(m_transactions.begin(), m_transactions.end(), [&hash](const WalletLegacyTransaction& tx) { return tx.hash == hash; });
...
void WalletUserTransactionsMoonBank::reset() {
  m_transactions.clear();
  m_transfers.clear();
  m_unconfirmedTransactions.reset();
}

std::vector<TransactionId> WalletUserTransactionsMoonBank::deleteOutdatedTransactions() {
...
void WalletUserTransactionsMoonBank::restoreTransactionOutputToDepositIndex() {
...
DepositId WalletUserTransactionsMoonBank::insertDeposit(const Deposit& deposit, size_t depositIndexInTransaction, const Hash& transactionHash) {
  DepositInfo info;
...
bool WalletUserTransactionsMoonBank::getDepositInTransactionInfo(DepositId depositId, Hash& transactionHash, uint32_t& outputInTransaction) {
  if (depositId >= m_deposits.size()) {
    return false;
  }
...
std::vector<DepositId> WalletUserTransactionsMoonBank::createNewDeposits(TransactionId creatingTransactionId, const std::vector<TransactionOutputInformation>& depositOutputs,
...
DepositId WalletUserTransactionsMoonBank::insertNewDeposit(const TransactionOutputInformation& depositOutput, TransactionId creatingTransactionId,
  const Currency& currency, uint32_t height) {
...
std::vector<DepositId> WalletUserTransactionsMoonBank::processSpentDeposits(TransactionId spendingTransactionId, const std::vector<TransactionOutputInformation>& spentDepositOutputs) {
  std::vector<DepositId> deposits;
...
DepositId WalletUserTransactionsMoonBank::getDepositId(const Hash& creatingTransactionHash, uint32_t outputInTransaction) {
...
std::vector<DepositId> WalletUserTransactionsMoonBank::getDepositIdsBySpendingTransaction(TransactionId transactionId) {
...
void WalletUserTransactionsMoonBank::addCreatedDeposit(DepositId id, uint64_t totalAmount) {
  m_unconfirmedTransactions.addCreatedDeposit(id, totalAmount);
}

void WalletUserTransactionsMoonBank::addDepositSpendingTransaction(const Hash& transactionHash, const UnconfirmedSpentDepositDetails& details) {
  m_unconfirmedTransactions.addDepositSpendingTransaction(transactionHash, details);
}

void WalletUserTransactionsMoonBank::eraseCreatedDeposit(DepositId id) {
  m_unconfirmedTransactions.eraseCreatedDeposit(id);
}
