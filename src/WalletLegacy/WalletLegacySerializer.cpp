...
WalletLegacySerializer::WalletLegacySerializer(CryptoNote::AccountBase& account, WalletUserTransactionsMoonBank& transactionsMoonBank) :
  account(account),
  transactionsMoonBank(transactionsMoonBank),
  walletSerializationVersion(WALLET_SERIALIZATION_VERSION)
{
}

void WalletLegacySerializer::serialize(std::ostream& stream, const std::string& password, bool saveDetailed, const std::string& moonbank) {
  ...
  if (saveDetailed) {
    serializer(transactionsMoonBank, "details");
  }

  serializer.binary(const_cast<std::string&>(moonbank), "moonbank");
...
void WalletLegacySerializer::deserialize(std::istream& stream, const std::string& password, std::string& moonbank) {
  ...
  if (detailsSaved) {
    if (version == 1) {
      transactionsMoonBank.deserializeLegacyV1(serializer);
    } else {
      serializer(transactionsMoonBank, "details");
    }
  }

  serializer.binary(moonbank, "moonbank");
}
