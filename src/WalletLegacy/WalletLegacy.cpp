...
WalletLegacy::WalletLegacy(const CryptoNote::Currency& currency, INode& node, Logging::ILogger& loggerGroup) :
  ...
  m_transactionsMoonBank(m_currency.mempoolTxLiveTime()),
  m_sender(nullptr),
...
WalletLegacy::~WalletLegacy() {
  ...
  {
    std::unique_lock<std::mutex> lock(m_moonbankMutex);
...
void WalletLegacy::initAndGenerate(const std::string& password) {
  {
    std::unique_lock<std::mutex> stateLock(m_moonbankMutex);
...
void WalletLegacy::initSync() {
  ...
  m_sender.reset(new WalletTransactionSender(m_currency, m_transactionsMoonBank, m_account.getAccountKeys(), *m_transferDetails, m_node));
  m_state = INITIALIZED;
...
void WalletLegacy::doLoad(std::istream& source) {
  ...
  try {
    ...
    std::string moonbank;
    WalletLegacySerializer serializer(m_account, m_transactionsMoonBank);
    serializer.deserialize(source, m_password, moonbank);
      
    initSync();

    try {
      if (!moonbank.empty()) {
        std::stringstream stream(moonbank);
        m_transfersSync.load(stream);
      }
    } catch (const std::exception&) {
      // ignore moonbank loading errors
    }
	  // Read all output keys moonbank
    ...
    runAtomic(m_moonbankMutex, [this] () {this->m_state = WalletLegacy::NOT_INITIALIZED;} );
...
std::vector<Payments> WalletLegacy::getTransactionsByPaymentIds(const std::vector<PaymentId>& paymentIds) const {
  return m_transactionsMoonBank.getTransactionsByPaymentIds(paymentIds);
}

void WalletLegacy::save(std::ostream& destination, bool saveDetailed, bool saveMoonBank) {
  if(m_isStopping) {
  ...
  m_asyncContextCounter.addAsyncContext();
  std::thread saver(&WalletLegacy::doSave, this, std::ref(destination), saveDetailed, saveMoonBank);
  saver.detach();
}

void WalletLegacy::doSave(std::ostream& destination, bool saveDetailed, bool saveMoonBank) {
  ContextCounterHolder counterHolder(m_asyncContextCounter);
  ...
  try {
    ...
    serializer.serialize(destination, m_password, saveDetailed, moonbank);

    m_state = INITIALIZED;
    m_blockchainSync.start(); //XXX: start can throw. what to do in this case?
    }
  catch (std::system_error& e) {
    runAtomic(m_moonbankMutex, [this] () {this->m_state = WalletLegacy::INITIALIZED;} );
...
std::error_code WalletLegacy::changePassword(const std::string& oldPassword, const std::string& newPassword) {
  std::unique_lock<std::mutex> passLock(m_moonbankMutex);
...
size_t WalletLegacy::getTransactionCount() {
  ...
  return m_transactionsMoonBank.getTransactionCount();
}
...
size_t WalletLegacy::getTransferCount() {
  ...
  return m_transactionsMoonBank.getTransferCount();
}
...
size_t WalletLegacy::getDepositCount() {
  ...
  return m_transactionsMoonBank.getDepositCount();
}
...
TransactionId WalletLegacy::findTransactionByTransferId(TransferId transferId) {
  ...
  return m_transactionsMoonBank.findTransactionByTransferId(transferId);
}
...
bool WalletLegacy::getTransaction(TransactionId transactionId, WalletLegacyTransaction& transaction) {
  ...
  return m_transactionsMoonBank.getTransaction(transactionId, transaction);
}
...
bool WalletLegacy::getTransactionByHash(const Crypto::Hash& hash, WalletLegacyTransaction& transaction) {
  ...
  return m_transactionsMoonBank.getTransactionByHash(hash, transaction); 
}
...
bool WalletLegacy::getTransfer(TransferId transferId, WalletLegacyTransfer& transfer) {
  ...
  return m_transactionsMoonBank.getTransfer(transferId, transfer);
}
...
bool WalletLegacy::getDeposit(DepositId depositId, Deposit& deposit) {
  ...
  return m_transactionsMoonBank.getDeposit(depositId, deposit);
}
...
uint64_t WalletLegacy::dustBalance() 
{
	...
	for (size_t i = 0; i < outputs.size(); ++i) 
  {
		...
		if (!m_transactionsMoonBank.isUsed(out)) 
    {
...
void WalletLegacy::onTransactionUpdated(ITransfersSubscription* object, const Hash& transactionHash) {
  ...
  if (m_transferDetails->getTransactionInformation(transactionHash, txInfo, &amountIn, &amountOut)) {
    ...
    events = m_transactionsMoonBank.onTransactionUpdated(txInfo, static_cast<int64_t>(amountOut)-static_cast<int64_t>(amountIn), newDepositOuts, spentDeposits, m_currency);
...
void WalletLegacy::onTransactionDeleted(ITransfersSubscription* object, const Hash& transactionHash) {
  ...
  {
    std::unique_lock<std::mutex> lock(m_moonbankMutex);
    events = m_transactionsMoonBank.onTransactionDeleted(transactionHash);
...
void WalletLegacy::onTransfersUnlocked(ITransfersSubscription* object, const std::vector<TransactionOutputInformation>& unlockedTransfers) {
  std::unique_lock<std::mutex> lock(m_moonbankMutex);
  auto unlockedDeposits = m_transactionsMoonBank.unlockDeposits(unlockedTransfers);
  lock.unlock();
...
void WalletLegacy::onTransfersLocked(ITransfersSubscription* object, const std::vector<TransactionOutputInformation>& lockedTransfers) {
  std::unique_lock<std::mutex> lock(m_moonbankMutex);
  auto lockedDeposits = m_transactionsMoonBank.lockDeposits(lockedTransfers);
  lock.unlock();
...
std::vector<TransactionId> WalletLegacy::deleteOutdatedUnconfirmedTransactions() {
  std::lock_guard<std::mutex> lock(m_moonbankMutex);
  return m_transactionsMoonBank.deleteOutdatedTransactions();
}
...
uint64_t WalletLegacy::calculateActualDepositBalance() {
  ...
  return calculateDepositsAmount(transfers, m_currency, heights) - m_transactionsMoonBank.countUnconfirmedSpentDepositsTotalAmount();
}
...
uint64_t WalletLegacy::calculateActualBalance() {
  return m_transferDetails->balance(ITransfersContainer::IncludeKeyUnlocked) -
    m_transactionsMoonBank.unconfrimedOutsAmount();
}

uint64_t WalletLegacy::calculatePendingBalance() {
  uint64_t change = m_transactionsMoonBank.unconfrimedOutsAmount() - m_transactionsMoonBank.unconfirmedTransactionsAmount();
  uint64_t spentDeposits = m_transactionsMoonBank.countUnconfirmedSpentDepositsProfit();
...
bool WalletLegacy::get_tx_key(Crypto::Hash& txid, Crypto::SecretKey& txSecretKey) {
  TransactionId ti = m_transactionsMoonBank.findTransactionByHash(txid);
