...
namespace
{
  ...
  std::unique_ptr<WalletLegacyEvent> makeCompleteEvent(WalletUserTransactionsMoonBank &transactionMoonBank, size_t transactionId, std::error_code ec)
  {
    transactionMoonBank.updateTransactionSendingState(transactionId, ec);
  ...
  uint64_t checkDepositsAndCalculateAmount(const std::vector<DepositId> &depositIds, const WalletUserTransactionsMoonBank &transactionsMoonBank)
  {
    uint64_t amount = 0;
    ...
    {
      Deposit deposit;
      throwIf(!transactionsMoonBank.getDeposit(id, deposit), error::DEPOSIT_DOESNOT_EXIST);
  ...
  void countDepositsTotalSumAndInterestSum(const std::vector<DepositId> &depositIds, WalletUserTransactionsMoonBank &depositsMoonBank,
                                           uint64_t &totalSum, uint64_t &interestsSum)
  {
    ...
    {
      Deposit &deposit = depositsMoonBank.getDeposit(id);
...
{
  WalletTransactionSender::WalletTransactionSender(const Currency &currency, WalletUserTransactionsMoonBank &transactionsMoonBank, AccountKeys keys, ITransfersContainer &transfersContainer, INode &node) : m_currency(currency),
                                                                                                                                                                                                       m_transactionsMoonBank(transactionsMoonBank),
                                                                                                                                                                                                       m_isStoping(false),
                                                                                                                                                                                                       m_keys(keys),
                                                                          ...
                                                                          uint64_t ttl)
  {
    ...
    transactionId = m_transactionsMoonBank.addNewTransaction(neededMoney, fee, extra, transfers, unlockTimestamp, messages);
                                                                             ...
                                                                             uint64_t mixIn)
  {
    ...
    transactionId = m_transactionsMoonBank.addNewTransaction(neededMoney, fee, std::string(), {}, 0, {});
    context->transactionId = transactionId;
    context->mixIn = mixIn;
                                                                                     ...
                                                                                     uint64_t fee)
  {
    ...
    transactionId = m_transactionsMoonBank.addNewTransaction(context->foundMoney, fee, std::string(), {}, 0, {});
    context->transactionId = transactionId;
    context->mixIn = 0;
                                                                  ...
                                                                  std::error_code ec)
  {
    ...
    if (ec)
    {
      events.push_back(makeCompleteEvent(m_transactionsMoonBank, context->transactionId, ec));
      return;
    }

    if (!checkIfEnoughMixins(context->outs, context->mixIn))
    {
      events.push_back(makeCompleteEvent(m_transactionsMoonBank, context->transactionId, make_error_code(error::MIXIN_COUNT_TOO_BIG)));
      return;
    }
                                                                            ...
                                                                            Crypto::SecretKey &transactionSK)
  {
    ...
    if (m_isStoping)
    {
      ...
      events.push_back(makeCompleteEvent(m_transactionsMoonBank, context->transactionId, make_error_code(error::TX_CANCELLED)));
    ...
    try
    {
      ...
      WalletLegacyTransaction &transaction = m_transactionsMoonBank.getTransaction(context->transactionId);
      ...
      m_transactionsMoonBank.updateTransaction(context->transactionId, tx, totalAmount, context->selectedTransfers, context->tx_key);
    ...
    {
      events.push_back(makeCompleteEvent(m_transactionsMoonBank, context->transactionId, ec.code()));
    }
    catch (std::exception &)
    {
      events.push_back(makeCompleteEvent(m_transactionsMoonBank, context->transactionId, make_error_code(error::INTERNAL_WALLET_ERROR)));
    }
  ...
  std::unique_ptr<WalletRequest> WalletTransactionSender::doSendMultisigTransaction(std::shared_ptr<SendTransactionContext> &&context, std::deque<std::unique_ptr<WalletLegacyEvent>> &events)
  {
    ...
    try
    {
      ...
      WalletLegacyTransaction &transactionInfo = m_transactionsMoonBank.getTransaction(context->transactionId);
      ...
      deposit.locked = true;
      DepositId depositId = m_transactionsMoonBank.insertDeposit(deposit, depositIndex, transaction->getTransactionHash());
      ...
      m_transactionsMoonBank.updateTransaction(context->transactionId, lowlevelTransaction, totalAmount, context->selectedTransfers, context->tx_key);
      m_transactionsMoonBank.addCreatedDeposit(depositId, deposit.amount + deposit.interest);
                                                                                           ...
                                                                                           std::deque<std::unique_ptr<WalletLegacyEvent>> &events, const std::vector<DepositId> &depositIds)
  {
    ...
    try
    {
      ...
      uint64_t totalSum;
      countDepositsTotalSumAndInterestSum(depositIds, m_transactionsMoonBank, totalSum, interestsSum);
      ...
      m_transactionsMoonBank.addDepositSpendingTransaction(transaction->getTransactionHash(), unconfirmed);
                                                   ...
                                                   std::vector<TransactionDestinationEntry> &splitted_dsts, uint64_t &dust)
  {
    ...
    {
      WalletLegacyTransfer &de = m_transactionsMoonBank.getTransfer(idx);
  ...
  {
    uint64_t unconfirmedOutsAmount = m_transactionsMoonBank.unconfrimedOutsAmount();
    uint64_t change = unconfirmedOutsAmount - m_transactionsMoonBank.unconfirmedTransactionsAmount();
  ...
  uint64_t WalletTransactionSender::selectNTransfersToSend(std::vector<TransactionOutputInformation> &selectedTransfers)
  {
    ...
    {
      if (!m_transactionsMoonBank.isUsed(outputs[i]))
      {
  ...
  uint64_t WalletTransactionSender::selectDepositTransfers(const std::vector<DepositId> &depositIds, std::vector<TransactionOutputInformation> &selectedTransfers)
  {
    ...
    for (auto id : depositIds)
    {
      ...
      throwIf(m_transactionsMoonBank.getDepositInTransactionInfo(id, transactionHash, outputInTransaction) == false, error::DEPOSIT_DOESNOT_EXIST);
      ...
      Deposit deposit;
      bool r = m_transactionsMoonBank.getDeposit(id, deposit);
      assert(r);
      if (r) {}
  ...
  void WalletTransactionSender::setSpendingTransactionToDeposits(TransactionId transactionId, const std::vector<DepositId> &depositIds)
  {
    for (auto id : depositIds)
    {
      Deposit &deposit = m_transactionsMoonBank.getDeposit(id);
