...
void ge_add(ge_p1p1 *r, const ge_p3 *p, const ge_moonbankd *q) {
  fe t0;
  fe_add(r->X, p->Y, p->X);
  fe_sub(r->Y, p->Y, p->X);
...
void ge_dsm_precomp(ge_dsmp r, const ge_p3 *s) {
  ge_p1p1 t;
  ge_p3 s2, u;
  ge_p3_to_moonbankd(&r[0], s);
  ge_p3_dbl(&t, s); ge_p1p1_to_p3(&s2, &t);
  ge_add(&t, &s2, &r[0]); ge_p1p1_to_p3(&u, &t); ge_p3_to_moonbankd(&r[1], &u);
  ge_add(&t, &s2, &r[1]); ge_p1p1_to_p3(&u, &t); ge_p3_to_moonbankd(&r[2], &u);
  ge_add(&t, &s2, &r[2]); ge_p1p1_to_p3(&u, &t); ge_p3_to_moonbankd(&r[3], &u);
  ge_add(&t, &s2, &r[3]); ge_p1p1_to_p3(&u, &t); ge_p3_to_moonbankd(&r[4], &u);
  ge_add(&t, &s2, &r[4]); ge_p1p1_to_p3(&u, &t); ge_p3_to_moonbankd(&r[5], &u);
  ge_add(&t, &s2, &r[5]); ge_p1p1_to_p3(&u, &t); ge_p3_to_moonbankd(&r[6], &u);
  ge_add(&t, &s2, &r[6]); ge_p1p1_to_p3(&u, &t); ge_p3_to_moonbankd(&r[7], &u);
}
...
/* From ge_p3_to_moonbankd.c */

/*
r = p
*/

void ge_p3_to_moonbankd(ge_moonbankd *r, const ge_p3 *p) {
...
void ge_sub(ge_p1p1 *r, const ge_p3 *p, const ge_moonbankd *q) {
  fe t0;
  fe_add(r->X, p->Y, p->X);
  fe_sub(r->Y, p->Y, p->X);
  fe_mul(r->Z, r->X, q->YminusX);
...
static void ge_moonbankd_0(ge_moonbankd *r) {
  fe_1(r->YplusX);
  fe_1(r->YminusX);
  fe_1(r->Z);
  fe_0(r->T2d);
...
static void ge_moonbankd_cmov(ge_moonbankd *t, const ge_moonbankd *u, unsigned char b) {
  fe_cmov(t->YplusX, u->YplusX, b);
  fe_cmov(t->YminusX, u->YminusX, b);
  fe_cmov(t->Z, u->Z, b);
...
void ge_scalarmult(ge_p2 *r, const unsigned char *a, const ge_p3 *A) {
  ...
  ge_p3_to_moonbankd(&Ai[0], A);
  for (i = 0; i < 7; i++) {
    ge_add(&t, A, &Ai[i]);
    ge_p1p1_to_p3(&u, &t);
    ge_p3_to_moonbankd(&Ai[i + 1], &u);
  }
  ...
  for (i = 63; i >= 0; i--) {
    ...
    ge_p1p1_to_p3(&u, &t);
    ge_moonbankd_0(&cur);
    ge_moonbankd_cmov(&cur, &Ai[0], equal(babs, 1));
    ge_moonbankd_cmov(&cur, &Ai[1], equal(babs, 2));
    ge_moonbankd_cmov(&cur, &Ai[2], equal(babs, 3));
    ge_moonbankd_cmov(&cur, &Ai[3], equal(babs, 4));
    ge_moonbankd_cmov(&cur, &Ai[4], equal(babs, 5));
    ge_moonbankd_cmov(&cur, &Ai[5], equal(babs, 6));
    ge_moonbankd_cmov(&cur, &Ai[6], equal(babs, 7));
    ge_moonbankd_cmov(&cur, &Ai[7], equal(babs, 8));
    ...
    ge_moonbankd_cmov(&cur, &minuscur, bnegative);
    ge_add(&t, &u, &cur);
    ge_p1p1_to_p2(r, &t);
  }
}
