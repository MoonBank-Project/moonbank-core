...
bool processServerAliasResponse(const std::string& s, std::string& address) {
  try {
    // https://openalias.org/
		// make sure the txt record has "oa1:bank" and find it
		auto pos = s.find("oa1:bank");
		if (pos == std::string::npos)
			return false;
...
simple_wallet::simple_wallet(System::Dispatcher& dispatcher, const CryptoNote::Currency& currency, Logging::LoggerManager& log) :
  ...
  m_consoleHandler.setHandler("reset", boost::bind(&simple_wallet::reset, this, _1), "Discard moonbank data and start synchronizing from the start");
...
bool simple_wallet::confirmTransaction(TransferCommand cmd, bool multiAddress) {
  ...
  /* 1000  = 0.001 $BANK */
  if (cmd.fee == 1000) {
    feeString = "0.001 $BANK (minimum)";
  } else {
    feeString = m_currency.formatAmount(cmd.fee) + " $BANK";
  }
  ...
  if (!multiAddress) {
    std::cout << "You are sending " << m_currency.formatAmount(cmd.dsts[0].amount)
              << " $BANK, with a fee of " << feeString << std::endl
...
bool simple_wallet::confirmDeposit(TransferCommand dcmd) {
  std::string feeString;
  std::string walletName = boost::filesystem::change_extension(m_wallet_file, "").string();
  feeString = m_currency.formatAmount(dcmd.fee) + " $BANK";
...
int main(int argc, char* argv[]) {
  ...
  bool r = command_line::handle_error_helper(desc_all, [&]() {
    ...
    if (command_line::get_arg(vm, command_line::arg_help)) {
      ...
      std::cout << "MoonBank Wallet v" << PROJECT_VERSION << std::endl;
      std::cout << "Usage: moonbank-wallet [--wallet-file=<file>|--generate-new-wallet=<file>] [--daemon-address=<host>:<port>] [<COMMAND>]";
  ...
  logger(INFO, BRIGHT_GREEN) << "MoonBank Wallet v" << PROJECT_VERSION;
